...
export function HistoryList() {
  ...
  if (!user) {
    return (
      <div className="mx-auto w-full max-w-4xl px-4 py-12 lg:px-6">
        <h1 className="font-mono text-xl font-semibold">Saved plans</h1>
        <p className="mt-3 text-sm text-muted-foreground">Sign in to view saved plans.</p>
  ...
  return (
    <div className="mx-auto w-full max-w-5xl space-y-5 px-4 py-6 lg:px-6">
      <div><h1 className="text-2xl font-semibold tracking-tight">Saved plans</h1></div>
      <div className="grid gap-3 border-y border-border py-4 sm:grid-cols-[1fr_12rem]">
        <label className="relative"><span className="sr-only">Search plans</span><Search className="pointer-events-none absolute left-3 top-1/2 size-4 -translate-y-1/2 text-muted-foreground" /><Input aria-label="Search plans" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search name or network" className="pl-9" /></label>
      ...
      {!isLoading && !isError && records.length === 0 ? <p className="border border-dashed border-border p-8 text-center text-sm text-muted-foreground">No saved plans.</p> : null}
